...
from aws import AWSService, AWSError
import request
...
class SimpleDB(AWSService):
	...
	def SendMessage(self, queueUrl, MessageBody):
		"""The SendMessage action delivers a message to the specified queue. The maximum allowed message size is 64 KB.
			...
			MessageBody -- The message to send.
				...
				Required: Yes
		"""
		r = request.AWSRequest(self._endpoint, '/', self._key, self._secret, 'SendMessage', {
			'Version': '2009-02-01',
	...
	def ReceiveMessage(self, queueUrl, AttributeNames=None, MaxNumberOfMessages=None, VisibilityTimeout=None):
		...
		r = request.AWSRequest(self._endpoint, '/', self._key, self._secret, 'ReceiveMessage', {
			'Version': '2009-02-01',
		}, self.Response_ReceiveMessage)
		if MaxNumberOfMessages is not None:
			r.parameters['MaxNumberOfMessages'] = MaxNumberOfMessages
		if VisibilityTimeout is not None:
			r.parameters['VisibilityTimeout'] = VisibilityTimeout
	...
	def DeleteMessage(self, receiptHandle):
		...
		r = request.AWSRequest(self._endpoint, '/', self._key, self._secret, 'DeleteMessage', {
			'Version': '2009-02-01',
